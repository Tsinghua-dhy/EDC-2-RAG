...
def _run_nli_GPT3turbo_batch(cases, topk):
    """
...
def process_slice(slice_cases):
    ...
    # 复制案例并删除嵌入数据
    cases = [deepcopy(case) for case in slice_cases]
    for case in cases:
    ...
    # 批量处理案例
    responses = _run_nli_GPT3turbo_batch(cases, topk)
...
def run(topk, noise):
    ...
    res_file = f"/your_path/{benchmark}/results/{date}_{dataset}_compress_{eval_method}_noise{noise}_topk{topk}.json"
    eval_method_1 = eval_method.split("_")[-1]
    case_file = f"/your_path/{benchmark}/datasets/case_{date}_{dataset}_summary_baseline_compress_{eval_method_1}_noise{noise}_topk{topk}.json"
...
for topk in topkk:
    for noise in noises:
        run(topk, noise)
        print(f"In Eval Baseline Compression: {topk}, {noise}")